  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold bg-gradient-brand bg-clip-text text-transparent">Бизнес сфера</span>
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <Link to="/jobs" className={`flex items-center space-x-1 text-sm font-medium transition-colors hover:text-primary ${location.pathname === '/jobs' ? 'text-primary' : 'text-muted-foreground'}`}>
